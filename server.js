...
// ── App modules ───────────────────────────────────────────────────────────────
const draftKitRouter = require("./draftkit/src/router");

// To add another app, require its modules and mount them below:
// const simchaKitRouter = require("./simchakit/src/router");
// const simchaKitWS     = require("./simchakit/src/ws");

// ── App registry ──────────────────────────────────────────────────────────────
// Populated after module imports below. Maps boardId prefix to WS handler.
var APP_WS_HANDLERS = {};
...
// Redirect bare root to the first registered app mount
app.get("/", (req, res) => {
  var firstPrefix = Object.keys(APP_WS_HANDLERS)[0];
  var firstMount  = firstPrefix ? "/" + firstPrefix.replace(/-$/, "") : "/logs";
  res.redirect(firstMount);
});

// ── Mount app routers ─────────────────────────────────────────────────────────
draftKitRouter.setLog(log);
app.use("/draft", express.static(path.join(__dirname, "draftkit", "public")));
app.use("/draft", draftKitRouter);
...
// Shared context passed to each app's WebSocket handler
var wsContext = {
  log:             log,
  wss:             wss,
...
// Inject broadcast into routers (for REST endpoints that need to push state)
var dkState = require("./draftkit/src/state");
var dkWS    = require("./draftkit/src/ws");

// Populate app registry
APP_WS_HANDLERS["draft-"] = dkWS;

// To add SimchaKit:
// var skState = require("./simchakit/src/state");
// var skWS    = require("./simchakit/src/ws");
// APP_WS_HANDLERS["simcha-"] = skWS;

draftKitRouter.setBroadcast(function(boardId) {
  var bs      = dkState.getBoardState(boardId, log);
  var payload = dkWS.statePayload(boardId, bs);
  broadcastToAll(boardId, payload);
});

// simchaKitRouter.setBroadcast(function(boardId) {
//   var bs      = skState.getBoardState(boardId, log);
//   var payload = skWS.statePayload(boardId, bs);
//   broadcastToAll(boardId, payload);
// });
...
wss.on("connection", function(ws) {
  ...
  ws.on("message", function(raw) {
    ...
    // Resolve which app handles this message by boardId prefix
    var boardId = msg.boardId || clientBoards.get(ws) || "";
    var handler = null;
    var prefixes = Object.keys(APP_WS_HANDLERS);
    for (var i = 0; i < prefixes.length; i++) {
      if (boardId.startsWith(prefixes[i])) {
        handler = APP_WS_HANDLERS[prefixes[i]];
        break;
      }
    }

    if (!handler) {
      log("WARN", "No handler found for boardId: " + boardId + " — ignoring");
      return;
    }

    handler.handleMessage(ws, msg, wsContext);
  });
...
server.listen(PORT, function() {
  ...
  Object.keys(APP_WS_HANDLERS).forEach(function(prefix) {
    var mount = "/" + prefix.replace(/-$/, "");
    log("INFO", "App [" + prefix + "]:  http://" + localIP + ":" + PORT + mount);
  });
